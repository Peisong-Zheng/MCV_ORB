...
2026-09-09。原写作计划已落实为 `main.tex` / `SI.tex` 的完整英文初稿。本文保留章节论证和图表设计的依据；三个新增正文图和五组 SI 表格均已生成，具体构建方式见 README。
...
引文：`lisiecki2005pliocene`、`bereiter2015revision`、`laskar2004long`；相位方法是本研究实现，圆周统计背景可引 `mardia2000directional`。`inso`/`paillard2026insolation` 只在真实输入生成过程能确认时用于来源，不作为当前拟合的默认依赖。正文简述，年代审计放 **Text S2、Table S2**。
...
引文：`davis2003observation`、`truccolo2005point`，实现引用 `virtanen2020scipy`。`kim2011granger` 可作似然比较方法背景，但本稿不称 Granger 因果检验。**Text S3、Figs. S8–S9、Table S3**。
...
**3.3 Barker supporting comparison（Fig. 2，详见 Figs. S5、S7、S9、S12）。** 主定义70事件，PI=0.08936，bootstrap p=0.0138，峰值336.91°、率比2.885；fixed59事件 PI=0.10423、峰值350.33°，仅点年代。用“相位响应大体一致”的补充语气，不称独立年代复制或更优检测法；不拿两目录原始 logL/p 比高低。

**3.4 Orbital alternatives and model robustness（Fig. 4）。** BG/Pre/Orb 三种嵌套比较分开解释。夏至日照与相位明显共享信息：控制日照后相位PI降至0.07870/0.03830；不能排除日照、偏心率或倾角的间接路径。额外九项比较的名义 Holm 校正与原主检验分开。设计网格、背景多项式、事件间隔记忆和分段相位检查用一段归纳，明细转 **Figs. S10–S11、Tables S4–S5**。`holm1979simple` 引于多重比较方法处。
...
3. **背景是否调节岁差（Figs. S13–S14）。** 预选 LR04 的两项交互；p=0.373/0.0657，Barker曲线有提示但证据不足。高LR04点拟合起伏更强不能独立证明交互，年代PI区间大于零也不代表显著。此结果用于讨论，不升为论文主结论。
...
|---|---|---|---|
| Fig. 1 | 哪些事件、什么年代、哪些区间是真正观测支持 | 新画0–400 kyr概览：事件与支持条带（NGRIP、MF/Sofular、Barker分行），以及对齐的岁差指数、LR04、CO2；主记录间隙不连接；Barker补充身份明确 | **已完成**：`figures/paper_summary/data_overview.pdf` |
| Fig. 2 | 无条件相位分布与条件效应是不同问题；两目录怎样比较 | 2×2：上行主目录、下行Barker；左列描述性相位分布，右列条件相位曲线；从两份主分析的b/c重绘；Barker保留variable/fixed两色；主定义bootstrap p放右列图注，fixed p只标nominal | **已完成重绘**：`figures/paper_summary/phase_comparison.pdf`，未重新拟合 |
...
图件生产输入：Fig.1 读取合成和Barker主目录、观测区间、两个主分析的 binned inputs（驱动曲线用完整Barker支持，同期一致性核对）；Fig.2 读取事件相位、拟合系数和 bootstrap summary；Fig.4 读取两套 orbital `comparison_summary.csv`。原始PDF不裁成位图、不修改原分析输出。所有图最终保存矢量PDF及PNG预览。
...
| Text S2 | 数据年代零点和三类工作年代模型 | Figs. S3–S5，Table S2；Rasmussen2022 dataset、Svensson2008、Moseley2020、Corrick2020、源石笋研究、Barker2011 |
| Text S3 | 分箱、older-bin历史、offset、相位、PI、拟合诊断、reduced bootstrap | Figs. S8–S9，Table S3；Davis2003、Truccolo2005、Mardia2000、Davison1997、SciPy2020 |
| Text S4 | 年代误差对PI、峰值与强度的影响 | Figs. S6–S7；有效分母、边界剔除、不得补抽，So-57替代和NGRIP网格敏感性用Table S4 |
| Text S5 | 主目录full-model抽样：A/B/C与相位展开、联合区域、误差来源差异 | 正文Fig.3及Table S3；不添加Barker尚未做的full-model精度分析；Davison1997 |
| Text S6 | 分箱/历史网格、背景形状、等待时间、分段相位与Barker定义 | Figs. S10–S12，Table S4；明确共同支持173/164.8与主180kyr的区别 |
| Text S7 | 其他驱动的十行比较／九项检验族、共线性、LR04调节 | 正文Fig.4、Figs. S13–S14、Table S5；Laskar2004、Holm1979，交互仅nominal |

SI 图固定编号：S1 MF–Sofular重叠；S2 MIS6合成定位；S3 NGRIP年代；S4 MIS6年代；S5 Barker年代；S6 合成年代PI；S7 Barker年代PI；S8 合成bootstrap；S9 Barker bootstrap；S10 设计敏感性；S11 气候形状/记忆；S12 Barker变量/固定阈值完整主图；S13/S14 两套LR04交互。

五组 SI 表已由 `paper_tables.py` 从保存结果导出为 CSV 和可编译 TeX，字段安排如下：

- **S1 目录与支持**：event ID、source label、年龄、source record、计时方法、来源；另列观测/响应支持，Barker与主目录分组。
- **S2 年代与单位审计**：源零点、转换、证据、误差量定义、工作分布、相关结构、边界规则；区分NGRIP MCE/2、model ext的研究假设、MF共享因子、Sofular深度warp、Barker uniform。
- **S3 主结果与不确定性**：两目录点估计与各自bootstrap、年代MC范围；主目录效应A/B/C另分栏标含义，不填不存在的Barker抽样区间。
- **S4 设定敏感性**：设计网格、非线性背景、记忆、分段相位、So-57和网格尺度；每项保留自己的N、暴露和检验口径，不跨支持比较AICc。
- **S5 轨道及交互**：每目录完整十行比较，新增九项Holm校正；两项LR04交互另列，保存df、PI、ΔAIC、nominal p及有效MC数。
...
沿用旧版 `monsoon_paper/main.tex`、`SI.tex`、`reference.bib`、`figures/Fig*.pdf`。新增本计划、引用审计、图件清单和简短README/Makefile。正文采用旧版 article+natbib/AGU bibliography 作为可编辑初稿，投稿前再切换官方模板。

已完成：参考文献核验；正文和 SI 全文；三个正文组合图；五组 SI 表；全部十八图同步。`make pdf` 先基于当前保存结果更新组合图和表格，再同步并编译；不重算科学模型，不从旧项目读取研究结果。

下一步是作者核稿与修订：检查主叙事、物理解释和术语；提供作者、单位、资助等真实信息；确定投稿模板并建立公开归档。当前初稿不追加 lag、逐周期、fixed-bootstrap 等新实验。详细本轮实施过程见 `docs/plans/2026-09-09-manuscript-draft.md`。
