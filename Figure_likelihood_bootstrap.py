...
SOURCES = (
    "figures/NGRIP_MIS6_likelihood_bootstrap/NGRIP_MIS6_likelihood_bootstrap.pdf",
    "Barker2011/figures/Barker2011_likelihood_bootstrap/Barker2011_likelihood_bootstrap.pdf",
)
...
def build_figure(project_root=PROJECT_ROOT):
    ...
    width, gap, heading_height = 180 / 25.4 * 72, 8, 17  # PDF coordinates are points.
    ...
    with plt.rc_context({"font.family": "sans-serif", "font.sans-serif": ["Arial", "DejaVu Sans"],
        ...
        for index, heading in enumerate(("NGRIP–MIS6", "Barker 2011")):
            ...
            overlay.text((index * (panel_width + gap) + 24) / width,
                         (height - 10) / height, heading, fontsize=9, weight="bold")
        stream = BytesIO()
        overlay.savefig(stream, format="pdf", transparent=True)
        plt.close(overlay)
    joined.merge_page(PdfReader(stream).pages[0])

    output = project_root / "figures/Figure_likelihood_bootstrap/Figure_likelihood_bootstrap.pdf"
